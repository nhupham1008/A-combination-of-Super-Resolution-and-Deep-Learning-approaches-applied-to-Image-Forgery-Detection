...
def print_results(model_mUNet, model_ResNet50_mUNet,data_df):
    ...
    for index,row in samples.iterrows():
        ...
        output_mUNet_res = model_mUNet.predict(image_arr).reshape(512,512,2)
        output_mUNet_res = np.argmax(output_mUNet_res, axis=-1)
        output_mUNet_res = np.expand_dims(output_mUNet_res, axis=-1)
        output_mUNet_res = output_mUNet_res*(255/3)
        output_mUNet_res = output_mUNet_res.astype(np.int32)

        output_ResNet50_mUNet_res = model_ResNet50_mUNet.predict(image_arr).reshape(512,512,2)
        output_ResNet50_mUNet_res = np.argmax(output_ResNet50_mUNet_res, axis=-1)
        output_ResNet50_mUNet_res = np.expand_dims(output_ResNet50_mUNet_res, axis=-1)
        output_ResNet50_mUNet_res = output_ResNet50_mUNet_res*(255/3)
        output_ResNet50_mUNet_res = output_ResNet50_mUNet_res.astype(np.int32)
        ...
        pred1 = np.concatenate([image1, image1, output_mUNet_res], axis=2)
        pred2 = np.concatenate([image1, image1, output_ResNet50_mUNet_res], axis=2)
        pred3 = np.concatenate([image1, image1, output_mUNet], axis=2)
        pred4 = np.concatenate([image1, image1, output_ResNet50_mUNet], axis=2)

        #final_image = np.concatenate([image, line, pred], axis=-3)
        input_img.append(image)
        GT_mask.append(gt)
        Predicted_mUNet_Res.append(pred1)
        Predicted_ResNet50_mUNet_Res.append(pred2)
        Predicted_mUNet.append(pred3)
        Predicted_ResNet50_mUNet.append(pred4)
    ...
    ax5.set_title("ResNet50+mUNet+Resolution")
    ax6 = plt.subplot(int(len(data_df)), 6,  6)
    ax6.set_title("Predicted ResNet50+mUNet")
    for i in range(len(data_df)):
        ...
        ax5.imshow(Predicted_ResNet50_mUNet_Res[i])
        #ax3.set_ylabel("Predicted Mask")
        ax6 = plt.subplot(int(len(data_df)), 6, 6 * i + 6)
        ax6.imshow(Predicted_ResNet50_mUNet[i])
    plt.show()
